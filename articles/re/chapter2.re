...
Jenkins Xはクラウドネイティブアプリケーションを開発するためのKubernetesネイティブな継続的インテグレーションおよび継続的デリバリプラットフォームです。Jenkins Xでアプリケーションを管理すると、アプリケーションがHelmでパッケージングされてKubernetesに展開する準備をしてくれます。利用者がアプリケーションを更新すると、その更新を検知してJenkins XはそのプレビューサイトをKubernetes上のstagingのnamespaceに用意し、本番環境へのPull Requestを作成してくれます。確認したらPull Requestをmergeするだけでアプリケーションを本番環境に展開することができます。ここらへんはGitOpsの概念が取り入れられています。
