...
通信を集約する「aggregator」には、全部のマイクロサービスを把握するという重要な役割があります。各マイクロサービスは、デプロイされた時点で親である「aggregator」に自分の情報を通知する、普通だったらこんな風に作りますよね。でもアプリ担当は考えました。
...
当初の計画では可視化ツールについてはあまり話題に上がっていませんでしたが、JDKのブース展示では来場者に可視化した様子をお伝えできればと @jyoshise が一晩でやってくれました。@jyoshise の j は ジェバンニ の j です。ちなみに導入は12月1日のことでした。まさに3日前。
