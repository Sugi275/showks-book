...
通信を集約する「aggregator」には、全部のマイクロサービスを把握するという重要な役割があります。各マイクロサービスは、デプロイされた時点で親である「aggregator」に自分の情報を通知する、普通だったらこんな風に作りますよね。普通だったら... でも、アプリ担当は考えました。
...
という仕様に落ち着いたのです。使ったのはKubernetes公式サポートのJavaScriptライブラリ。とても良くできていて機能にも不足なし！ただし使い方はソースコードに書いてあるからね、JavaScriptだし大丈夫だよね、まさにフルサポート（泣。こうして、人知れずAPI呼び出しのデバッグに明け暮れる、というより暮れて明ける夜が続いたのでした。

真面目な話、「Watch」を使ったインスタンスのデプロイ状態の監視とアトリビュートの取得処理は、Kubernetes APIを使った自動化の参考になることでしょう。showks-aggregatorのソースコードもお見逃しなく。
