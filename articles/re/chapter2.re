...
前述の通り、Kubernetesはアプリケーションの実行状態をマニフェストというYAMLファイルで定義するものであり、今回はそのマニフェストはHelmにより生成されます。つまり、個々のアプリケーションにとってのInfrastructure as Codeであるマニフェストもまた、アプリケーションのビルド成果物ということになります。
...
その後、開発が終わったとの想定でstagingからmasterブランチに対してPull Requestを投げて貰います。Concourse CIがそれを検知し、不正なリポジトリからPull Requestが送られていないかを確認し、テストした後問題が無ければLGTMのコメント後、マージを行います。
...
通信を集約する「aggregator」には、全部のマイクロサービスを把握するという重要な役割があります。各マイクロサービスは、デプロイされた時点で親である「aggregator」に自分の情報を通知する、普通だったらこんな風に作りますよね。でもアプリチームは考えました。

 * 各マイクロサービスには、インフラのことを知らないままでいてほしい（世間ではplatform independentという）
 * なにより「Kubernetesらしさ」を出したい！

入れられるなら可能な限り自分の趣味を入れる・・・ ではなくて、どうやったら皆さんのCloud Native/Kubernetes理解の役に立てるのかを考えるのが担当の使命です。こうして、やや強引にも


@<b>{「aggregator」はKubernetes APIを使ってマイクロサービスの情報を取得する}


という仕様に落ち着いたのです。使ったのはKubernetes公式サポートのJavaScriptライブラリ。非常によくできたライブラリで、Kuberenetes APIのほぼ全機能をカバーしているようです。ただし使い方はソースコードに書いてあるからね、JavaScriptだし大丈夫だよね、のまさにフルサポート（泣。こうして、人知れずAPI呼び出しのデバッグに明け暮れる、というより暮れて明ける夜が続いたのでした。
