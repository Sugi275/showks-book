...
Japan Container Days（JKD） v18.12が開催されたのは12月ですが、実行委員会が組織されたのはそれより半年以上前。なんせ1000人近くを集めるカンファレンスですから、準備には結構時間がかかるのです。
...
現在、さまざまなクラウドベンダーがKubernetesのマネジメントサービスを提供しています。オンプレにKubernetesを構築するソリューションも多く登場しています。 こういった各サービス・ソフトウェア間を跨がるソフトウェアを作り、相互運用性を確認するような企画はどうかと。
...
前述したように、僕らが参考にしたShowNetはネットワーク機器の相互運用性を確認する企画でした。ネットワーク機器は、あらかじめ定められているネットワークプロトコルを扱えるよう各ベンダーがそれぞれの実装を行っています。規格は定められているもの、実装はベンダーに委ねられているため、規格を満たしていない・規格の解釈違い・実装のバグなどが起こりえます。そのため、同じ規格に対応しているはずなのに上手く繋がらないという事象が頻繁に起こります。これらの問題を確認する場として、ShowNetは機能しているのです。

一方、僕らがやろうとしていたshowKsは、どのベンダーのサービスであっても、基本的に同一のコードベースを利用しているはずなのです。ベンダー独自の拡張についても、KubernetesはCustomResourceDefinitionという仕組みで拡張可能のため、Kubernetes本体をフォークする必要性があまり無いのですね。強いて言えばバージョン違いくらいのものでしょうか。

なので、あるマネージドKubernetesで動いているアプリケーションは、そのまま他のKubernetesに移行できてしまいます。PersistentVolumeやLoadBalancerなどの差分はありますが、それらの差分を修正するのはそれほど難しいことではありません。
 ...
 * 誰でも書ける簡単なGoのアプリケーションを想定（"hello, hogehoge"を書き換えるレベル）
...
さて、無事リブートしたshowKs。ペンギン界で最もクラウドネイティブな@jacopen主導のもとオンライン・オフラインで何度か打ち合わせを行った結果、当初よりもアプリケーション部分を膨らませたいという気持ちになって来ました。メンバーのひとりである@jyoshiseから、以下のようなアイディアが提案されたのです。
 ...
 * キャンバスには参加者固有のSNSアカウントやメッセージなどを埋め込めることができる
