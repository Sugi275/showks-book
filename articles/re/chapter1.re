...
//indepimage[01/pre1][フォームに入力したら][scale=0.5]
//indepimage[01/pre2][あなた専用のキャンバスが出来ていて][scale=0.5]

GitHubリポジトリやCIパイプラインやCDパイプラインが出来ている。

//indepimage[01/pre3][GitHubリポジトリや][scale=0.5]
//indepimage[01/pre4][CIパイプラインや][scale=0.5]
//indepimage[01/pre5][CDパイプラインが出来ている][scale=0.5]
//indepimage[01/pre6][そしてポータルでみんなの絵が見られる][scale=0.5]
 ...
 * キャンバスには自由にペンで書き込むことができる
 * 参加者全体のキャンバスが一カ所に集められ、俯瞰して見ることのできるポータルとなる
 ...
 ** CI/CDパイプライン
 ** PullRequestベースの開発
 * お絵描きアプリというテーマで@<b>{共同作業感}を出す
 * アプリ・マニフェスト・ドキュメント @<b>{全てお持ち帰り可能}
