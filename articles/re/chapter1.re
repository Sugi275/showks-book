...
なので、あるManaged Kubernetesで動いているアプリケーションは、そのまま他のKubernetesに移行できてしまいます。PersistentVolumeやLoadBalancerなどの差分はありますが、それらの差分を修正するのはそれほど難しいことではありません。
...
さて、無事リブートしたshowKs。オンライン・オフラインで何度か打ち合わせを行った結果、当初よりもアプリケーション部分を膨らませたいという気持ちになって来ました。メンバーのひとりである@jyoshiseから、以下のようなアイディアが提案されたのです。
