...
スケーラブルで堅牢なアーキテクチャをマイクロサービスで実現してCloud Nativeアプリ開発らしさを出すという目的は、今回のshowKsで概ね達成できました。しかし、カンファレンスという特殊な環境において、わかりやすい展示になることを優先させた部分は少なからずありました。そのためアプリケーションの構成が、実際の企業のシナリオとは少し異なったものとなってしまったかもしれません。
...
次の機会では、Cloud Nativeアプリ開発のトレンドをうまく反映して、実際の企業シナリオにすぐにでも適用できるようなデモができることを目指したいと思います。
...
クラウドネイティブの定義の一つに復元力というものがありましたよね？showKsはクラウドネイティブなアプリケーションですから、ちょっとやそっとの障害で動かなくなってしまっては困ります。その為には意図的にサーバーやコンテナを落としてみて、システムにどのような影響があるのか？SPOFになっている部分は無いか？を常に把握しておき、意図しない本物の障害が起きたとしても被害を最小限にするようにしたいものです。このような考え方はChaos Engineering@<fn>{chaosengineering}と言われています。
